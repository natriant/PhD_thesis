...
%% plots are produced: cernbox/Project_thesis/scripts_for_simple_figures_plots. and for Fig.6 /eos/user/n/natriant/pyheadtail_data/9July2021
...
\end{equation}
where $f_k$ lies in a range of positive frequencies and $\fourierxform{\alpha}(f_k)$ and $\fourierxform{\phi}(f_k)$ are the discrete Fourier transforms of the modulation signals $\alpha(t)$ and $\phi(t)$ respectively. The units of the $G_\alpha(f_k)$ are V$^2$/Hz and the units of $G_\phi(f_k)$ are rad$^2$/Hz.
...
As follows from the discussion in section~\ref{subsec:Measure_noise}, phase and amplitude noise can be represented by discrete time series $\phi_n = \phi(n\Delta t)$ and  $\alpha = \alpha(n\Delta t)$ respectively, so that the crab cavity (CC) instantaneous voltage is given by:
...
\begin{figure}[!ht]
    ...
    \begin{subfigure}[t]{0.42\textwidth}
        \centering
        \includegraphics[width=1\textwidth]{./images/app_B/coast1_setting2_10logL.png}
    ...
    \begin{subfigure}[t]{0.42\textwidth}
        \centering
        \includegraphics[width=1\textwidth]{./images/app_B/coast1_setting2_Gyy_linear_interpolant.png}
...
\begin{figure}[!ht]
     ...
     \end{subfigure}
     \hfill
     \caption{Comparison between emittance growth found from simulations in PyHEADTAIL and emittance growth expected from an analytical model~\cite{CC-Mastoridis-Baudrenghien}.  The emittance growth is driven by amplitude and phase noise, with kicks in the simulations generated from a measured power spectrum.}
