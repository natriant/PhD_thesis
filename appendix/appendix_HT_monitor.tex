...
The $\CC$ voltage is then reconstructed from the $\CC$ kick which is written as $\theta = - \frac{q V_\mathrm{CC}(t)}{\symE}$, where $q$ is the charge of the particle, $\symE$ the beam energy and $V_\mathrm{CC}(t) = V_\mathrm{CC} \sin(2 \pi \CCfrequency t + \phi_\mathrm{CC}) $ is the voltage that a particle experiences while passing through the $\CC$. In the context where the HT monitor measures the signal as a function of time, $t$, the voltage in the above formula is expressed accordingly as $V_\mathrm{CC}(t)$, where $t=0$ the center of the bunch.
