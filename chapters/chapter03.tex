...
The theoretical formalism for predicting the transverse emittance growth in the presence of $\CC$ RF amplitude and phase noise was derived in~\cite{PhysRevSTAB.18.101001}. The derivation assumes a single bunch is crossing the crab cavity and the phase of the CC is set such that the zero-crossing of the voltage coincides with the longitudinal center of the bunch, as well as no coupling between the transverse planes and constant energy (no acceleration). % top of right column in https://journals.aps.org/prab/pdf/10.1103/PhysRevSTAB.18.101001 paper.
