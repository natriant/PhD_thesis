...
In synchrotrons the charged particles are controlled with electromagentic fields. The motion of a particle with charge $q$ and velosity $\mathbf{v}$ moving in an electric field $\mathbf{E}$ and a magnetic field $\mathbf{B}$ is defined by the influence of the Lorentz force: 
...
The co-ordinate system used to describe the motion of the beam particles around the accelerator is illustrated in Fig.~\ref{fig:coordinate_system}. The red line shows the ideal trajectory around the machine, which is predetermined by the construction of the accelerator and is called the designed orbit or reference trajectory. However, the individual particles do not follow the reference trajectory: an example trajectory is shown with the blue line. The individual trajectories of the particles are described by the co-ordinate system $(\mathbf{e_x, e_y, e_s})$ whose origin moves along the reference trajectory. The variable $s$ denotes the distance along the reference trajectory. In accelerator physics, $s$ is usually chosen as the independent variable instead of time. %why? p.19 https://www.bnl.gov/isd/documents/74289.pdf

The motion of the particles is seperated in the transverse and longitudinal planes. In the transverse plane, $(\mathbf{e_x, e_y})$, the motion is orthogonal to the reference trajectory. For any given location $s$ the horizontal $x$ and vertical $y$ position of a particle are expressed relative to the reference trajectory. In the longitudinal plane, $(\mathbf{e_s})$, the motion is tangential to the reference trajectory and the variable $z$ indicates the longitudianal position of the particle at any given location $s$ along the ring. % Or see Andy's explanation for low emittance storage rings: https://cds.cern.ch/record/1982424/plots




. In accelerator physics it is more common to use 

At any given time $t$, which basically corresponds to a position $s$, the horizontal and vertical position of a particle are expressed relative to the reference trajectory.

Usually in accelerator physics the In accelerator physics it is more commont to  


The distance along the reference trajectory is parameterised by the independent variable s. 

Therefore, at any given time the horizontal and vertical position of a particle are expressed relative to the reference trajectory.



- reference particle reference orbit
...
\begin{figure}[!h] % at the directory of ipac22
    ...
    \includegraphics[width=0.8\textwidth]{images/Ch2/coordinates_particle_motion.png}
        \caption{Co-ordinate system used to describe particles motion in a synchrotron. ey ex es are unit vectors in a moving co-ordinate system which follows the particle along the ring,.}
        \label{fig:coordinate_system}
 \end{figure}



 The distance along the reference trajectory is parameterised by the independent variable s. At any point along the reference trajectory, the transverse position of a particle is specified by Cartesian co-ordinates in a plane perpendicular to the reference trajectory.

 %https://en.wikipedia.org/wiki/Unit_vector. Curvilinear system. Xavier's thesis Fig.2.1
...
First principles --> Lorentz force
