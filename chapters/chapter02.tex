...
\begin{enumerate}
    ...
    \end{equation}
    where $i=1, ..., N$ with N being the macroparticles number and the matrix $M$ is given by:
        ...
        M = \begin{pmatrix}
            ...
            \end{pmatrix} \begin{pmatrix}
                ...
                \end{pmatrix}  \begin{pmatrix}
                    ...
                    \end{pmatrix} 
    \end{equation}
    where $\alpha_{x, \mathrm{IP0/IP1}}$ and $\beta{x, \mathrm{IP0/IP1}}$ are the optic paramters at the interaction points IP0 and IP1 respectively and $\Delta \mu_{x, \mathrm{IP0 \to IP1 }}$ the phase advance from IP0 to IP1. As smooth approximation is used, the phase advance equals: 
    ...
    \item \textbf{Chromaticity and detuning with transverse amplitude:} The chromaticity (up to higher orders) and amplitude detuning are implemented as a change of the phase advance of each individual particle as follows:
    \begin{equation}
        \Delta
    \end{equation}
    
     and they are included in the linear matrix $M$ of the transverse tracking mentioned in the previous step. For example, the change of the phase advance for a macroparticle $i$ from 
