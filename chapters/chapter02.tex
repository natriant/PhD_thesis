...
The motion of the particles is separated in the transverse and longitudinal planes. In the transverse plane, $(\mathbf{e_x, e_y})$, the motion is orthogonal to the reference trajectory. For any given location $s$ the horizontal $x$ and the vertical $y$ position of a particle are expressed relative to the reference trajectory. In the longitudinal plane, $(\mathbf{e_s})$, the motion is tangential to the reference trajectory. The variable $z$ indicates the longitudinal position of the particle at any given location $s$ along the ring. % Or see Andy's explanation for low emittance storage rings: https://cds.cern.ch/record/1982424/plots
...
\begin{equation}
    x^\prime = \frac{dx}{ds} = \frac{dx}{dt}\frac{dt}{ds} = \frac{u_x}{u_z} =  \frac{p_x}{p_z} = \frac{p_x}{p_0}
\end{equation}


and $p_0$ is the momentum of the reference or synchronous particle: the one that travels on the reference trajectory. Its coordinates are ($x=y=z=0$) and its momentum is ($p_x=p_y=0, p_z=p_0$) with $p_0 = \gammarel m_0 u_z$. 
...
Start here: we describe with 6D coordinates more accurate.

- To have a more accurate representation 6D phase space is used.\\
- x' or px
