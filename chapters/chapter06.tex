...
The measured spectra of Fig... were converted to a discrete time series that can be used in the numerical simulations as described in Appendix~\ref{sec:measured_spectra_to_time_series}.
...
By looking at the plots, it is evdient that there is significantly less spread between the emittance values over the different runs. This is due to the fact that as discussed in Appendix~\ref{sec:measured_spectra_to_time_series}, in the time series generated by the measured spectra the random factor is included in the set of random phases which leads to much less deviation on the values than the sequence of white noise kicks where the random factor is in their amplitude. To this end, the simualtion here is repeated for just 10 different runs to reduce the uncertainty.

Last, the same simulation is repeated in the presence of both amplitude and phase noise together.
