...
The $\CC$ element installed at a location $s_0$, acts like a single dipole field error with strength depending on the longitudinal position $z$ of the particle. To this end, the induced orbit shift from the $\CC$ element as detected at another location $s_1$ is benchmarked against the theoretically expected orbit shift. %The latter has already been discussed in the context of the reconstruction of $\CC$ voltage from the HT monitor in Section~\ref{sec:Vcc_calibration}. 
