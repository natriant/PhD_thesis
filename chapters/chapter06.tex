...
The simulation studies presneted in Section~\ref{subsec:sixtracklib_kicks_transverse_angle} were repeated here but this time the real $\CC$ elements were implementated and switched ON. Both $\CC$1 and $\CC2$ operated at 1\,MV, for $\phi_\mathrm{CC1}$=90\,deg and  $\phi_\mathrm{C2C}2$=270\,deg. Figure~\ref{fig:sixtracklib_CC1_vs_CC2_orbit_shift_vs_theory} visulises that the orbit shifts from two $\CC$s operating in opposite phase cancel out as expected. The results shown are obtained from Sixtracklib simualtions that were conducted following the exact same procedure with the equivalent study presented in Section~\ref{subsec:sixtracklig_CC_implementation} (see Fig.~\ref{fig:sixtracklib_CC_orbit_shift_vs_theory}.)
