...
As menitoned above, one of the main concerns regarding the $\CC$ operation with protons is the emittance growth due to noise in their RF system as it leads to luminosity loss. For the HL-LHC case the target values regarding the luminosity loss and emittance growth are very tight. In particular the targeted luminosity loss due to $\CC$ RF noise induced emittance growth is targeted at just 1$\%$. This corresponds to an $\CC$ RF noise induced emittance growth of 2 $\mathrm{\%/h}$.* footnote: these rates with the current cc specifications for hl-lch and the impact of transverse damper. 

Given these very tight values it is very important to obtain good understanding on this phenomemenon 


ii order to be able to predict and define limites etc.

This thesis focuses on understanding and evaluating the mechanism of $\CC$ RF noise induced emittance growth including both numerical and experimental studies. The studies presented in this thesis were conducted for the SPS machine, for which we are able to have operational experience.
