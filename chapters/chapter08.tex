...
\begin{table}[!hbt]
   ...
   \end{centering} \footnotesize{$^\ast$ These value corresponds to the requested intial value at the start of each coast. Regarding the intensity, the intensity drop was found to be insignificant for all the coasts.  \\ $^\dagger$ This value corresponds to the average rms measured bunch length over all the coasts of 2022. \\$^\ddagger$ Here the rms betatron tune spread includes only the contribution from the detuning with amplitude present in the SPS machine. More details along with the calculations for the listed values can be found in Appendix~\ref{app:detuning_with_amplitude}.}
   \end{minipage}
\end{table}
