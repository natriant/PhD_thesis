...
Last, only one $\CC$, $\CC 2$, was used for simplicity and it operated at 1\,MV with zero phase offset, $\phiCC=0$. These values were validated with the HT monitor (post-processing procedure desribed in Chapter~\ref{Ch:CC_set_up}). Unfortunately, only one beam based measurement of the $\CC$ voltage is available whcih is displayed in Fig. ... It is clear that the measured values of voltage amplitude and phase are very close to the requested ones.
...
\begin{table}[!hbt]
   ...
   \caption{Main machine and beam parameters for the emittance growth studies with CCs in SPS in 2018.}
	\begin{tabu} to \textwidth {X[c,m] X[0.5c,m] X[0.5c,m] X[0.01c,m]}
      ...
      \multicolumn{2}{l}{$\CC 2$ voltage / phase / frequency, $\VCC$ / $\phiCC$ / $\fCC$}  & \multicolumn{2}{c}{1\,MV / 0\, rad / 400.78\,MHz} \\
      \multicolumn{2}{l}{Number of proton per bunch, $\Nb$} & \multicolumn{2}{c}{3 $\times 10^{10}$ p/b$^\ast$} \\
      ...
      \multicolumn{2}{l}{Horizontal / Vertical normalised emittance, $\emitx$ / $\emity$}  & \multicolumn{2}{c}{2\,ns / 2\,ns$^\ast$}\\
   ...
   \end{centering} \footnotesize{$^\ast$ The value correspond to the requested intial value at the start of each coast. The measured evolution of the parameter through the experiment is presented in the Sections~\ref{sec:emit_growth_meas_2018} and~\ref{sec:bunch_length_intensity_meas_2018}.}
   \end{minipage}
\end{table}
...
dose pasa gia to injected noise below.

- change of coast when emittance reaches big. See Section 6.1.

\section{Injected RF noise}\label{sec:injected_RF_noise}

Before showing the emittance growth results, the injected RF noise is showed.
