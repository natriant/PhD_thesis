...
The second configuration will be referred to as "coast". As discussed in the Section~\ref{subsec:cern_sps}, coast is the operational mode where the bunches circulate at the machine for long periods at constant energy (maximum 270\,GeV). This configuration succeed the acceleration cycle during which the bunches reach the required energy. During coast the magnetic field of the main dipoles ramains also constant. This configuration was used for the noise-induced emittance growth measurements where the objective was to study the long term emittance evolution. 

The evolution of the dipole magnetic fields (in arbitrary units) during the two operational configurations that were set in the SPS up for the $\CC$ tests is illustrated in Fig.~\ref{fig:sps_cycles_cc_tests}. The corresponding beam energy is also noted on the plots. It is worth commenting that the magnetic field value in the coast mode (right) is plotted for a time scale that is equivalent to the one of the accelerator cycle for easier comparison and consistency. In reality, during coast the dipole fields remain at constant strength and field for longer periods (typically 30 minutes - 1 hour).  
