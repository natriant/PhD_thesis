...
%\subsection{SPS operational setup in the presence of Crab Cavities}\label{sec:sps_cc_setup}
%For the emittance growth studies with $\CC$s two special operational configurations were set up in the SPS. %Both configurations were used with a limited number of bunches (up to 4 bunches, see Chapter~\ref{Ch:2018_analyisis}). 

%In the first configuration which will be referred to as the "acceleration cycle", the field of the main dipole magnets ramps up so that it accelerates the bunches from the injection energy of 26\,GeV up to 270\,GeV. The bunches remain at 270\,GeV for a few seconds before being dumped. After that, the field of the SPS main dipole magnets returns to the initial value and the procedure is repeated. The first part of the acceleration cycle, before the energy ramp up, is called "flat bottom", while its last part where the bunches remain at 270\,GeV is called "flat top". This terminology is introduced here as it will facilitate the explanation of some concepts and procedures in the following. 
%This configuration was used to accelerate the bunches to 270\,GeV required for the emittance growth measurements. Furthermore, it was used for the beam-based measurement of the $\CC$ voltage (see Section.. and Appendix~\ref{ch:app_HT_monitor}).

%The second configuration will be referred to as "coast". As discussed in Section~\ref{subsec:cern_sps}, coast is the operational mode where the bunches circulate at the machine for long periods at constant energy (maximum 270\,GeV). This configuration succeeds the acceleration cycle during which the bunches reach the required energy. During coast, the magnetic field of the main dipoles remains also constant. This configuration was used for the noise-induced emittance growth measurements where the objective was to study the long-term emittance evolution. 

%The evolution of the dipole magnetic fields (in arbitrary units) during the two operational configurations that were set in the SPS up for the $\CC$ tests is illustrated in Fig.~\ref{fig:sps_cycles_cc_tests}. The corresponding beam energy is also noted on the plots. It is worth commenting that the magnetic field value in the coast mode (right) is plotted for a time scale that is equivalent to the one of the accelerator cycle for easier comparison and consistency. In reality, during coast, the dipole fields remain at constant strength and field for longer periods (typically between 30 minutes and 1 hour).

% Path 2 figures: /eos/user/n/natriant/2022/SPS_MDs_2022/cc_md_16May2022/magnetic_cycle_before_coast_MD4/nxcals_MD4_cycle.ipynb
%\begin{figure}[htp]
%   \begin{subfigure}{.45\textwidth}
%       \centering
%       \includegraphics[width=.95\linewidth]{images/Ch5/MD4_cycle_16May2022_aribtrary_units_for_thesis.png}  
       %\caption{$k_\mathrm{LOD}=+15 \ \mathrm{/m^{4}}$}
       %\label{fig:cc_md_2022_coast6}
 %  \end{subfigure}
 %  \begin{subfigure}{.45\textwidth}
 %          \centering
 %          \includegraphics[width=.95\linewidth]{images/Ch5/MD4_cycle_13Oct2021_coast_aribtrary_units_for_thesis.png}  
           %\caption{$k_\mathrm{LOD}=-5 \ \mathrm{/m^{4}}$}
           %\label{fig:cc_md_2022_coast9}
%   \end{subfigure}
 %  \caption{Magnetic field of the main dipoles and corresponding beam energy for the two operational configurations set up in the SPS for the emittance growth studies with CCs.} %\textit{Left}:  \textit{Right}:}
 %  \label{fig:sps_cycles_cc_tests}
%\end{figure}
...
SPS receives the proton beam at 26\,GeV from the PS. It was found that the ramp to 270\,GeV (coast energy) could not be performed with the $\CC$ switched on, since during the acceleration one of the vertical betatron sidebands was crossing the $\CC$ frequecny resulting to resonance excitation and beam loss~\cite{BE_seminar, CC_rephasing_RF}. Changing the $\CC$ frequnecy during the ramp so that it stays synchronous with the beam was not an option due to hardware limitations~\cite{CC_rephasing_RF}. % CC not fast enough.
...
It was important to ensure that during the "coast" the beam will experience the same kick from the $\CC$ each turn. In other words the SPS main RF system operating at $\sim$200\,MHz needed to be synchronous with the $\CC$ operating at 400.78\,MHz. Due to the larger bandwidth of the SPS main RF system the $\CC$ was used as a master. Therefore the $\CC$ was operating at a fixed frequency, while the main accelerating cavities were adjusted to the exact half of the $\CC$ frequency so that they become synchronous with the crabbing signal. For the studies at 270\,GeV the synchronisation took place at the end of the energy ramp shortly a few seconds after the $\CC$ was switched on~\cite{BE_seminar}. %(refer to Fig.~\ref{fig:sps_cycles_cc_tests} for the parts of the acceleration cycle). 
...
The beam-based measurement of the $\CC$ voltage (reconstructed from the Head-Tail monitor) is displayed in Fig.~\ref{fig:crabbing_sin_fit_MD5} with red color. The voltage amplitude, $\CCvoltage$, is obtained from a sinusoidal fit on the reconstructed voltage, $V_\mathrm{CC}(t)$, from the Head-Tail monitor reading. The standard procedure of least squares fitting (see Appendix section~\ref{app:non_linear_fitting}) is followed. In particular, $\VCC (t)$ is fitted with the folllowing three-parameter ($\CCvoltage$, $\phi_\mathrm{CC}$, $d$) model function which also provides the $\CC$ phase and voltage offset:
\begin{equation}\label{eq:sin_fit_cc}
   f(x) = \CCvoltage \sin{(2\pi \CCfrequency x + \phi_\mathrm{CC})} + d ,
\end{equation}
where $\VCC$ is the amplitude of the $\CC$ voltage, $ \phi_\mathrm{CC}$ the $\CC$ phase and $d$ the vertical voltage offset. The fit is performed for a fixed $\CC$ frequency, as the operational value is fixed and well known, $\CCfrequency$=400.78\,MHz. 

The offset parameter, $d$, is added to the model function as measurements from other dedicated studies\footnote{A total of seven different dedicated experiments with $\CC$s took place in SPS in 2018 in order to address various questions on the operation of the $\CC$ with proton beams~\cite{BE_seminar}. The emittance growth experiment was the fifth in chronological order.} that took place during this first experimental campaign with $\CC$S in the SPS have shown that the reconstructed $\CC$ voltage, $V_\mathrm{CC}(t)$, is not centered around zero in the vertical axis (see Appendix Figs.~\ref{fig:VCC_from_HT_monitor_measurement} and~\ref{fig:crabbing_reconstruction_HT_monitor}). A possible explanation for the assymetry is that it is a results of the cable response of the Head-Tail monitor pick up~\cite{Levens_WP2_HT_CC_diagnostic}.

seems to be a result of the HT monitor pick up and cable response~\cite{Levens_WP2_HT_CC_diagnostic}. However its origin is not yet fully understood and will have to be addressed in the future.
...
\begin{figure}[!h]
   ...
   \includegraphics[width=0.7\textwidth]{images/Ch5/HT_VCC_callibration_20180905_135033_sin_fit_fixed_freq_thesis.png}
       \caption{Demonstration of the sinusoidal fit on the Head-Tail monitor reading in order to obtain the CC parameters as described in Section~\ref{sec:CC_voltage_meas}. The fit results, are given in the yellow box. The measured voltage amplitude, $V_{CC}$, was found to be 0.99\,MV while its uncertainty, $\Delta V_{CC}$, was measured at 0.04\,MV. The measured voltage value agrees well with the requested value of 1\,MV.}
       \label{fig:crabbing_sin_fit_MD5}
\end{figure}


This procedure was optimised in the experimental campaign of 2022 (see chapter..)
In order to obtain the peak voltage 

shortly after the bunches are accelerated at 270 GeV but before switching to coast mode for reasons that are discussed in appendix.

(post-processing procedure desribed in Appendix~\ref{ch:app_HT_monitor}). Unfortunately, only one beam based measurement of the $\CC$ voltage is available which is displayed in Fig.~\ref{fig:crabbing_sin_fit_MD5}. It is clear that the measured value of voltage amplitude, $\VCC$ = 0.99 $\pm$ 0.04\,MV , is in good agreement with the requested one. It should be noted, that due to the beam energy of 270\,GeV the crabbing is less visible than the example discussed in Chapter~\ref{Ch:CC_set_up} (see Fig.~\ref{fig:crabbing_sin_fit_MD2}) for 26\,GeV. Therefore, here the part of the signal that is used for the fit is the one for which the normalised sum signal (black dashed line) is above 0.2 (instead of 0.4 that was the condition for the case of 26\,GeV)


Last, only one $\CC$, $\CC 2$, was used for simplicity and it operated at 1\,MV. This value was validated with the Head-Tail monitor (post-processing procedure desribed in Appendix~\ref{ch:app_HT_monitor}). Unfortunately, only one beam based measurement of the $\CC$ voltage is available which is displayed in Fig.~\ref{fig:crabbing_sin_fit_MD5}. It is clear that the measured value of voltage amplitude, $\VCC$ = 0.99 $\pm$ 0.04\,MV , is in good agreement with the requested one. It should be noted, that due to the beam energy of 270\,GeV the crabbing is less visible than the example discussed in Chapter~\ref{Ch:CC_set_up} (see Fig.~\ref{fig:crabbing_sin_fit_MD2}) for 26\,GeV. Therefore, here the part of the signal that is used for the fit is the one for which the normalised sum signal (black dashed line) is above 0.2 (instead of 0.4 that was the condition for the case of 26\,GeV)
...
The noise injected in the $\CC$ RF system was a mixture of amplitude and phase noise up to 10\,kHz, overlapping and primarily exciting the first betatron sideband at $\sim 7.8$\,kHz \footnote{The frequency in Hz of the first betatron sidebands (vertical plane) is computed according to Eq.~\eqref{eq:phase_noise_kick} for $k=0$: $\pm$ $\widebar{q}_y \times f_\mathrm{rev}$ = 0.18$\times$ 43.38\,kHz$\approx$ 7.8\,kHz.}. The phase noise was always dominant. The noise levels were measured with a spectrum analyzer E5052B~\cite{E5052B_insight} and are expressed as $10\log_{10}\mathcal{L}(f)$\,[dBc/Hz]. The relation between the measured noise levels and the power spectral densities (PSDs) in Eq.~\eqref{eq:dey_an} and Eq.~\eqref{eq:dey_pn} is given by $S_\Delta = 2\mathcal{L}(f)$, with $S_{\Delta A}$ in\,1/Hz and $S_{\Delta \phi}$ in $\mathrm{rad^2/Hz}$. This relation is extensively discussed in Appendix~\ref{ch:app_B} and specifically in~\ref{app:Measured_noise}}. Figure~\ref{fig:example_PN_and_AN_coast1_setting2} displays an example measurement of amplitude (left) and phase (right) noise acquired during the experiment.
