...
\begin{equation}\label{eq:4p_gauss}
   f(x) = k + Y e^{-\frac{(x-\mu)^2}{2 \sigma_u^2}},
\end{equation}
where $k$ is the signal offset of the photomultiplier, $Y$ is the signal amplitude, $\mu$ is the mean of the gaussian distribution and $\sigma_u$ its standard deviation. The uncertainty of the measured rms beam size, $\Delta \sigma_u$, is defined as the error of the fit of the $\sigma_u$ parameter (see Appendix~\ref{app:non_linear_fitting}).
