...
From Fig.~\ref{fig:MD5_overview_x_y_sigma_t}, it becomes clear that bunches 2,3, and 4 are longitudinally unstable. This is believed to be due to the
...
which was not affected by the instability. Nevetherless, in the next paragraph the intensity measurements for all the four bunches are exceptionally illustrated.

\subsection{Intensity measurements}\label{subsec:intensity_2018_meas}
The bunch by bunch intensity measurements that were performed along the experiment with induced $\CC$ noise are displayed in Fig.~\ref{fig:MD5_overview_intensity}. In particular the intensity values normalised with the intial value are shown for each bunch. The four different bunches are inficated with the four different colors. The acquisitions from both the ABWLM and the Wall Current Monitor are shown and it becomes clear that they are in a very good agreement.

More significant losses are observed for the longitudinally unstable bunches (bunch 2,3, and 4). However, they are not of concern as they are not included in the following analysis.

Compute percentage per hour and say that was as expected without noise.
