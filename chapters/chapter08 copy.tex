...
For the experiment described here, the emittance growth was measured in the presence of four different noise levels (as close as possible with the ones used in 2018) with the Landau octupoles switched off. The objective of this experiment was a) to reproduce the scaling of emittance growth observed in 2018 (see Fig.~\ref{fig:MD5_summary_plot}) and b) to benchmark the expected suppression factor from PyHEADTAIL simulations with the impedance model. 



The experiment described here was performed with injected artificial noise in the RF system of $\CC$1 for noise power as close as possible to the settings use in 2018. 

The objective of the experiment described here was to measure the emittance growth in the presence of phase noise with power as close as possible to the values used in 2018 u 
...
The reason behind this is that during the $\CC$ phase scan that was performed in 2022 to calibrate the $\CC$ phase offset (see Section~\ref{subsec:cc_calibration_2022}) $\CC$2 tripped systematically. The issue is associated with the change of the RF phase but fixing this problem would have been time-consuming which was not an option due to the very limited machine time of the experiment. Therefore, for the emittance growth measurements in 2022 $\CC$1 was used.
