...
The simulation setup in PyHEADTAIL and the parameters used can be found in Section~\ref{sec:setup_simulations_emit_growth}. To have the clearest visualisation of the effect, the emittance growth was induced by a dipolar noise kick (instead of a $\CC$ phase noise kick). Furthermore, the vertical amplitude detuning coefficient was set to $\alpha_\mathrm{yy}=6000$\,1/m which corresponds to a regime of strong suppression (see Fig.~\ref{fig:study_5_dipole_noise}). Last, the linear chromaticity was corrected to zero, $Q^\prime_{x,y}$=0, such as it does not introduce any additional tune spread in order to simplify the observables.


- Add noise strength value. 
- range of intensity values.



that the emittance growth simulations were conducted 


in the presence of a dipolar noise kick (instead of $\CC$ phase noise kick) for more clear visualisation of the effect. Additionally, the chromaticity
