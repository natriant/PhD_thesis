...
\begin{figure}[!h] 
 ...
 The study was conducted over a range of bunch intensities from 0 to $5 \times 10^{10}$ protons per bunch. This range was chosen to be in the vicinity of the $\CC$ experiments in SPS in 2018, where the initial intensity was $3 \times 10^{10}$ - $3.5 \times 10^{10}$ protons per bunch. The initial intensity was measured with the Fast Beam Current Transformer (FBCT) and the Beam Current Transformer (in particular with the device SPS.BCTDC.41435) which are installed in SPS. Both instruments have the same working principle but different bandwidths. They measure the beam induced current in their ferrite core and transform it to the number of protons per beam. Fruther details on their working principle and operation can be found in~\cite{Jones:1982418, Jakob:624188}.
