...
$\CC$ phase noise was applied as it was the dominant type in the 2018 experiment, with power spectral density of $1.68 \times 10^{-10} \mathrm{rad^2/Hz}$ \footnote{It corresponds to $A=10^{-8}$ for the scaling factor of Eq.~\eqref{eq:phase_noise_kick}}. For this noise power a growth rate of about 25\,nm/s is expected (exciting the first betatron sidebands at $\pm$7.8~\,kHz \footnote{The frequency in Hz of the first betatron sidebands (vertical plane) is computed according to Eq.~\eqref{eq:phase_noise_kick} for $k=0$: $\pm$ $\widebar{q}_y \times f_\mathrm{rev}$ = 0.18$\times$ 43.45\,kHz$\approx$ 7.8\,kHz.}). 
