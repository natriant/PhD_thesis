...
\begin{figure}[!h] % at the directory of ipac22
 ...
 The difference in the PyHEADTAIL simulations between the $\CC$ RF phase and amplitude noise could by explained by the fact that the phase noise (which is similar to a dipolar noise kick but with a high order distortion) is associated with the head-tail mode 0, while the amplitude noise is associated with the head-tail mode 1. The hypothesis, that the emittance growth suppression from impedance induced effects is related to head-tail mode 0 will be discussed in detail in the folllowing sections and was also tested experimentally in 2022.
...
\begin{figure}[!ht]
 ...
 Comparing Fig.~\ref{fig:CC_200MHz_amplitude_phase_noise} (right) and Fig.~\ref{fig:MD_2018_impedance_simulations} it can be seen that emittance growth driven by $\CC$ phase noise in the absence of wakefield kicks (blue) is in excellent agreement for the $\CC$ operating at 200\,MHz and 400\,MHz, except for the case with zero amplitude detuning, $\alpha_\mathrm{yy}=0$, where for $f_\mathrm{CC}=200$\,MHz, almost zero emittance growth is observed. This could be explained by the fact that for $f_\mathrm{CC}$=200\,MHz, the $\CC$ wavelength ($\sim$5\,ns) is much longer than the bunch length ($4\sigma_t$=1.7\,ns) hence almost only the linear part of the $\CC$ kick affects the bunch  which does not lead to phase mixing of the bunch particles and hence emittance growth. This reasoning is in agreement with the discussion in Section~\ref{subsec:amplitude_detuning_scan}.
 ...
 Repeating the last comparison but in the presence of wakefield kicks (orange) it can be seen that emittance growth driven by $\CC$ phase noise is in good agreement with the results for the $\CC$ operating at 200\,MHz and 400\,MHz. Suppression of the emittance growth, which depends on the amplitude detuning, is observed in both cases. However, in the case of phase noise and $f_\mathrm{CC}$=200\,MHz the emittance growth suppression appears stronger and reaches up to a factor of 10 instead of just 4-5 in the case of $f_\mathrm{CC}$=400\,MHz. This observation agrees with the hypothesis that the emittance growth suppression by impedance induced effects is related to rigid bunch motion (head-tail mode 0). In particular, since for $f_\mathrm{CC}$=200\,MHz, the $\CC$ wavelength is much longer than the bunch length the $\CC$ kick is closer to a pure dipole excitation. 
...
Figure~\ref{fig:study_5_dipole_noise} shows the noise-induced vertical emittance growth as a function of amplitude-dependent tune spread with and without the presence of wakefield kicks. In the absence of wakefields (blue) the emittance growth appears independent of the tune spread while for $\alpha_\mathrm{yy}=0$ zero emittance growth is observed. The latter observation is expected, since the emittance growth is caused by decoherence induced by the betatron tune spread. Therefore for zero tune spread no emittance growth is anticipated. 
...
In the folllowing paragraphs, the suppression mechanism of the $\CC$ RF noise induced emittance growth suppression by the transverse impedance is further explored with tracking simulations using PyHEADTAIL. 
...
\begin{figure}[!h] 
 ...
 The study was conducted over a range of bunch intensities from 0 to $5 \times 10^{10}$ protons per bunch. This range was chosen to be similar to the $\CC$ experiments in SPS in 2018, where the initial intensity for each coast was $3 \times 10^{10}$ - $3.5 \times 10^{10}$ protons per bunch. The initial intensity was measured with the Fast Beam Current Transformer (FBCT) and the Beam Current Transformer (in particular with the device SPS.BCTDC.41435) which are installed in SPS. Both devices have the same working principle but different bandwidths. They measure the beam induced current in their ferrite core and transform it to the number of protons per beam. Fruther details on their working principle and operation can be found in~\cite{Jones:1982418, Jakob:624188}.
