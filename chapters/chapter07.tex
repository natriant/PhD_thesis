...
\begin{figure}[htp]
    ...
    \end{subfigure}
    \caption{Transverse emittance growth driven by  RF phase noise at 400\,MHz without (blue) and with (orange) the impedance effects is shown for five different value of linear chromaticity increasing from top left to bottom right.}
    \label{fig:study_6_chroma_scan}
\end{figure}

In the absence of impedance effects (blue), the $\CC$ RF noise-induced emitttance growth rates appear independent of both the amplitude detuning coefficient and the value of linear chromaticity. This is in agreement, with the predictions of the theory developed from T.~Mastoridis and P.~Baudrenghien~\cite{PhysRevSTAB.18.101001} for white noise spectrum.
