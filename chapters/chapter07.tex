...
In the above-mentioned section, it was shown that in order to reproduce the realistic rms tune spread ($\sim 2-3 10^{-4}$) that was present in SPS during the 2018 experiments (from intrinsic non-linearities), the vertical amplitude detuning coefficient should be $| \alpha_\mathrm{yy} |$=2000\,1/m. However, it becomes evident from Fig.~\ref{fig:MD_2018_impedance_simulations} that this leads to the regime of very strong suppression, which might toughen up this parametric study. Therefore, the intensity scan was performed for $\alpha_\mathrm{yy}$=6000\,1/m which leads to a regime of medium suppression but is still close enough to the realistic machine conditions of 2018.
...
\begin{figure}[!h] 
    ...
    \includegraphics[width=0.7\textwidth]{images/Ch7/sps_270GeV_PN1e-8_400MHz_QpxQpy5e-1_ayy6000_intensity_spsCCtests.png}
        \caption{PyHEADTAIL simulations illustrating the effect of the beam intensity on the transverse emittance growth driven by CC RF phase noise in the presence of impedance effects as a function of the bunch intensity. The blue vertical line shows the intensity value during the SPS CC tests in 2018.}
...
\subsection{Spectral analysis of the bunch centroid motion}\label{subsec:spectral_analysis}
In this subsection, the incoherent spectrum of the bunch is investigated aiming to visualise and hence confirm that the suppression mechanism is a result of the separation of the coherent mode from the incoherent spectrum. The incoherent spectrum can be obtained from Fourier's analysis of the turn-by-turn centroid motion. This method can be also found in the bibliography as Schottky noise method~\cite{Caspers:1213284} and is often used for beam diagnostics as it can reveal information on important parameters such as the revolution frequency, the momentum spread, and the incoherent spectrum. In the following, the turn-by-turn position of the bunch was obtained with PyHEADTAIL tracking simulations.

The simulations here, were performed for the beam and machine conditions of 2018 (listed in Tables~\ref{tab:pyheadtail_simulation_parameters} and~\ref{tab:CC_pyheadtail_simulation_parameters}) in accordance with the studies presented in Section~\ref{sec:first_obs_suppression} including the SPS imepdance model. However, for the spectral analysis studies no noise kick was applied on the bunch to minimise the external perturbations and obtain a clear frequency signal. This choice is justified, since the noise kicks do not have any impact on the frequency domain. Nevertheless, the six-dimensional Gaussian bunch distribution was generated with an initial vertical offset of 0.2 times the vertical beam size. The reason behind this is that due to the offset the beam will undergo betatron oscillations with higher amplitude which will facilitate the Fourier analysis. Furthermore, this type of simualtion requires longer tracking than the emittance growth studies, $10^6$ turns instead of $10^5$, for better representation in the frequency domain. Last, $5 \times 10^4$ macroparticles were sufficient \footnote{Following similar studies in Ref.~\cite{Zorzano-Mier:446334} and exploratory studies confirmed that the number of macroparticles used does not affect the quality of the results.} for this type of simulation which reduced significantly the computational time of the simulation. 

Below, the spectra were obtained by applying a NAFF algorithm to the turn-by-turn data as described in Section~\ref{subsec:test_implementation_pyheatail}. Last, the amplitude of the spectral components is expressed as power spectral density in units of $\mathrm{rad^2}/Q_y$ as it is preferred over the amplitude of the spectral components of the Fourier transform which is in arbitrary units.

Below, the frequency spectrum of the bunch vertical centroid motion is studied for the following two values of the vertical detuning coefficient, $\alpha_\mathrm{yy}$ = -2000 and -20000\,1/m since they correspond to regimes of strong and zero suppression respectively (as illustrated in Fig.~\ref{fig:MD_2018_impedance_simulations}). The value $\alpha_\mathrm{yy}$ = -2000\,1/m results to tune spread similar to the one that was present in the SPS during the $\CC$ tests in 2018. The spectra are shown along with the transverse emittance growth for the corresponding amplitude detuning value as simulated for the studies in Section~\ref{sec:first_obs_suppression}.
...
It is evident that for the conditions of the emittance growq



the emittance growth experiments in the SPS the small tune shift from the

\newpage



for the same set-up and parameters described



To test the suppression mechanism the incoherent spectrum of the centroid motion of the bunch is obtained for  



Here, the incoherent spectrum of the bunch is investigated for different intensities in an attempt to visualise and hence confirm that the suppression mechanism is a result of the separation of the coherent mode from the incoherent spectrum. 

 The simulation setup was done as described in Section~\ref{sec:setup_simulations_emit_growth}. No noise kick was applied on the bunch to minimise the external perturbations and obtain a clear frequency signal. The initial bunch was generated with Gaussian distributions in transverse and longitudinal planes and an initial vertical offset of 0.2 times the vertical beam size. Additionally, and for reasons that were described in Section~\ref{subsec:intensity_scan_emit_growth} the simulations took place with vertical detuning coefficient at $\alpha_\mathrm{yy}$=6000\,1/m and chromaticity corrected to zero, $Q^\prime_{x,y}$=0. $5 \times 10^4$ macroparticles were sufficient \footnote{Following similar studies in Ref.~\cite{Zorzano-Mier:446334} and exploratory studies confirmed that the number of macroparticles used does not affect the quality of the results.} for this type of simulation which reduced significantly the computational time of the simulation. The spectrum was obtained with a long tracking $10^6$ turns and by applying an FFT algorithm \footnote{In particular a NAFF algorithm (available in NAFFlib python library) is used as described in Section~\ref{subsec:test_implementation_pyheatail}.}.
 
The results are displayed in Fig.~\ref{fig:study_9_shottky_noise_intensity_scan} which shows the vertical spectrum of the centroid (coherent) oscillation of the bunch for different intensity values, selected from the range studied in the previous subsection: ~\ref{subsec:intensity_scan_emit_growth}. Each subplot shows the power spectral density of the motion of the centroid as a function of the frequency in tune units for a given intensity value, which increases from the top left to the bottom right. The power spectral density of the motion of the centroid is obtained from the above-mentioned Fourier transform using Eq.~\eqref{eq:Sxx_definition_discrete_normalized}. 


\subsection{Spectral analysis of the centroid motion}\label{subsec:spectral_analysis}
Here, the incoherent spectrum of the bunch is investigated for different intensities in an attempt to visualise and hence confirm that the suppression mechanism is a result of the separation of the coherent mode from the incoherent spectrum. The incoherent spectrum is obtained from Fourier's analysis of the turn-by-turn centroid motion. This method can be also found in the bibliography as Schottky noise method~\cite{Caspers:1213284} and is often used for beam diagnostics as it can reveal information on important parameters such as the revolution frequency, the momentum spread, and of course the incoherent spectrum.

 The simulation setup was done as described in Section~\ref{sec:setup_simulations_emit_growth}. No noise kick was applied on the bunch to minimise the external perturbations and obtain a clear frequency signal. The initial bunch was generated with Gaussian distributions in transverse and longitudinal planes and an initial vertical offset of 0.2 times the vertical beam size. Additionally, and for reasons that were described in Section~\ref{subsec:intensity_scan_emit_growth} the simulations took place with vertical detuning coefficient at $\alpha_\mathrm{yy}$=6000\,1/m and chromaticity corrected to zero, $Q^\prime_{x,y}$=0. $5 \times 10^4$ macroparticles were sufficient \footnote{Following similar studies in Ref.~\cite{Zorzano-Mier:446334} and exploratory studies confirmed that the number of macroparticles used does not affect the quality of the results.} for this type of simulation which reduced significantly the computational time of the simulation. The spectrum was obtained with a long tracking $10^6$ turns and by applying an FFT algorithm \footnote{In particular a NAFF algorithm (available in NAFFlib python library) is used as described in Section~\ref{subsec:test_implementation_pyheatail}.}.
 
The results are displayed in Fig.~\ref{fig:study_9_shottky_noise_intensity_scan} which shows the vertical spectrum of the centroid (coherent) oscillation of the bunch for different intensity values, selected from the range studied in the previous subsection: ~\ref{subsec:intensity_scan_emit_growth}. Each subplot shows the power spectral density of the motion of the centroid as a function of the frequency in tune units for a given intensity value, which increases from the top left to the bottom right. The power spectral density of the motion of the centroid is obtained from the above-mentioned Fourier transform using Eq.~\eqref{eq:Sxx_definition_discrete_normalized}. Expressing the amplitude of the spectral components as power spectral density in units of $\mathrm{rad^2}/Q_y$ is preferable to display the amplitude of the spectral compoenents of the Fourier transform which is in arbitrary units.
 ...
 \begin{figure}[htp]
    ...
    \label{fig:study_9_shottky_noise_intensity_scan}
\end{figure}


Two upper plots correspond to the regime of no suppression, midd in the cut off and intensity dependent regime, bottom saturation of dependence on intnesity.

The coherent betatron mode clearly emerges from the incoherent spectrum for higher intensities, at the time the emittance growth suppression starts to be visible too. However, once the coherent mode is completely seperated from the incoherent spectrum the dependence on the intensity saturates.

According, to Y.~Alexahin's theory
...
- check the presentation for IPAC and copy the text from there.
...
Dipole noise, or phase noise. (both?)
