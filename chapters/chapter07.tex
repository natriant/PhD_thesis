...
Fig.. shows the vertical spectrum of the centroid (coherent) oscillation of a bunch simulated with PyHEADTAIL for different intensity values. The simulation setup was done as described in Section~\ref{sec:setup_simulations_emit_growth}. No noise kick was applied on the bunch to minimise the external perturbations and obtain a clear frequency signal. The initial bunch was generated with Gaussian distributions in transverse and lon- gitudinal planes and an initial vertical offset of 0.2 times the vertical beam size. Additionally, and for reasons that were described in Section~\ref{subsec:intensity_scan_emit_growth} the simulations took place with vertical detuning coefficient at $\alpha_\mathrm{yy}$=6000\,1/m and chromaticity corrected to zero, $Q^\prime_{x,y}$=0. The spectrum was obtained with a long tracking $10^6$ turns and applying an FFT algorithm \footnote{In particular a NAFF algorithm (available in NAFFlib python library) is used as described in Section~\ref{subsec:test_implementation_pyheatail}.}. Last, it is worth mentioning that $5 \times 10^4$ macroparticles were sufficient \footnote{In accordance with similar studies in Ref.~\cite{Zorzano-Mier:446334} and exploratory studies confirmed tha the number of macroparticles used does not affect the quality of the results.} for this type of simualtion which reduced significantly the computational time of the simualtion. 


Long tracking for $10^6$ turns, number of particles


= (it was cross checked with convergence studies that it does not affect hte results.) and apply a fft algormin (naff, refrence to also before) on the motion of the centroid.

To validate the suppression mechanism, one should look at the incoherent spectrum of the bunch. 

To validate the suppression mechanism, the Scho

- check yuri alexahin for vocubulary.
- show all the plots.
