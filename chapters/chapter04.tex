...
It is clear from Fig.~\ref{fig:VCC_from_HT_monitor_measurement} that the reconstructed $\CC$ voltage, $\VCC(t)$, is not centered around zero. The amplitude of the signal is 


It should be noted here that the reconstructed $\CC$ voltage appears to be assymetric around $\VCC (t)$ = 0\,MV. Thus, the measured $\VCC$ is considered as half the peak to peak amplitude i.e. the difference between the positive and the negative peaks of the signal.
