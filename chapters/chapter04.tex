...
In the first configuration which will be referred to as "acceleration cycle", the field of the main dipole magnets ramps up so that it accelerates the bunches from the injection energy of 26\,GeV up to 270\,GeV. The bunches remain at 270\,GeV for a few seconds before being dumped. After that the field of the SPS main dipole magnets returns to the initial value and the procedure is repeated. The first part of the acceleration cycle, before the energy ramp up, is called "flat bottom", while its last part where the bunches remain at 270\,GeV is called "flat top". This terminology is introduced here as it will facilitate the explanation of some concepts and procedures in the following sections of this chapter. This configuration was used for the calibration of the $\CC$ phase offset and for the beam based measurement of the voltage (see Section ..). Additionally, it was used for the preparation of the beam for the second operational mode which is called "coast".

As discussed in the Section~\ref{subsec:cern_sps}, coast is the operational mode where the bunches circulate at the machine for long periods at constant energy (maximum 270\,GeV). This configuration can be used right after the accelerating cycle during which the bunches reached the required energy. During coast the magnetic field of the main dipoles ramain also constant. This configuration was used for the noise-induced emittance growth measurements where the objective was to study the long term emittance evolution.
...
\begin{figure}[htp]
   ...
   \begin{subfigure}{.45\textwidth}
       \centering
       \includegraphics[width=.95\linewidth]{images/Ch4/MD4_cycle_16May2022_aribtrary_units_for_thesis.png}  
       %\caption{$k_\mathrm{LOD}=+15 \ \mathrm{/m^{4}}$}
       %\label{fig:cc_md_2022_coast6}
   \end{subfigure}
   \begin{subfigure}{.45\textwidth}
           \centering
           \includegraphics[width=.95\linewidth]{images/Ch4/MD4_cycle_13Oct2021_coast_aribtrary_units_for_thesis.png}  
...
SPS receives the proton beam at 26\,GeV from the PS. It was found that the ramp to 270\,GeV could not be performed with the $\CC$ switched on, since during the acceleration one of the vertical betatron sidebands was crossing the $\CC$ frequcny resulting to resonance excitation and beam loss~\cite{BE_seminar, CC_rephasing_RF}. Changing the $\CC$ frequnecy during the ramp so that it stays synchronous with the beam was not an option due to hardware limitations~\cite{CC_rephasing_RF}. % CC not fast enough.
