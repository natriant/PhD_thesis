...
\begin{figure}[!h]
   ...
   \includegraphics[width=0.65\textwidth]{images/Ch4/Vcc_amplitude_uncertainty.pdf}
       \caption{Illustration of the CC amplitude voltage, $V_{CC}$, defined as half the peak to peak amplitude, $V_{CC, p-p}$ (blue) and of its uncertainty, $\Delta V_{CC}$, defined as the offset needed for the signal to be zero-centered.}
...
where $k$ is the signal offset of the PM, $A$ is the signal amplitude, $\mu$ is the mean of the Gaussian distribution and $\sigma$ its standard deviation. The uncertainty of the measured beam size, $\Delta \sigma$, is defined as the error on the standard deviation, $\sigma$, which is computed from the square root of the diagonal elements of the covariant matrix.
