...
\subsection{General infortmation}\label{subsec:HT_general_info}
...
% 2. lxplus: cernbox/2021/CC_MD_2021_summary/HT_monitor/2018_HT_monitor/MD2_phase_scan
\begin{figure}[!h]
\centering         
...
% The analysis of the HT signals in this section is done with the following script: cernbox/2021/CC_MD_2021_summary/HT_monitor/2018_HT_monitor/MD2_phase_scan
...
\subsection{Estimation of the amplitude and the uncertainty of the measurement}\label{subsec:def_amplt_uncertainty}

It is clear from Fig.~\ref{fig:VCC_from_HT_monitor_measurement} that the reconstructed $\CC$ voltage, $\VCC(t)$, is not centered around zero. This voltage offset seems to be a result of calibration issue of the HT monitor as it didn't change for measurements with the same $\CC$ and beam conditions. 

In this case, the amplitude of the measured signal is defined as half the peak to peak amplitude, $\VCC = V_{CC, p-p}/2$. Note that the peak to peak amplitude, $V_{CC, p-p}$, is the difference between the maximum positive and negative peaks of the signal.

The uncertainty in the amplitude, $\Delta \VCC$, is defined as the offset that needs to be added or subtracted to the signal, such as it is centered around zero. In other words, it is defined as half the sum of the maximum positive and negative peaks of the signal. It should be noted that this is a systematic uncertainty. Figure~\ref{fig:VCC_amplitude_and_uncertainty} shows the peak to peak amplitude, $V_{CC, p-p}$, the amplitude of the signal, $\VCC$, and the uncertainty, $\Delta \VCC$, for the reconstructed $\CC$ voltage, $\VCC(t)$.
...
The impact of the $\CC$ phase on the voltage experienced by the beam and on the uncertainty of its measurement was also studied experimentally. The results are summarised in Fig .. . The phase is extracted from a sinusoidal fit on the reconstracted $\CC$ votlage from the HT monitor reading after the offset is removed (Fig..). Clearly the, with increasing phase ...
...
it becomes clear that the $\CC$ phase has an important impact on the voltage experienced by the beam and on the uncertainty of its measurement. In particular, ...

The phase is obtained from a sinusoidal fit. Show example of sinusoidal fit (after the removal of the offset). From the same data as above.

A non linear mean square minimisation (error, is as big as the phase itself. what should I comment on it?)

Plots (maybe also with horizontal error on the measured phase?):
Amplitude vs phase
Phase offset 
