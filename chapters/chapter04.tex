...
It should be noted here, that the measured intra-bunch offset, $\Delta y_{, HT}$, is inserted in Eq.~\eqref{eq:CC_orbit_shift_Chao} after removing the baseline and converting it in mm. Figure~\ref{fig:VCC_from_HT_monitor_measurement} illustrates the cavity voltage computed from the HT signals discussed in Section~\ref{sec:HT_info}. Note that the x-axis here doesn't correspond to the 25\,ns as previously but to the bunch length with $t=0$ at zero crossing. The corresponding optic parameters are listed in Table~\ref{tab:SPS_HT_CC}
