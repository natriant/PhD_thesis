...
where $k$ is the signal offset of the PM, $A$ is the signal amplitude, $\mu$ is the mean of the Gaussian distribution and $\sigma$ its standard deviation. A non-linear least square minimization is used to fit the gaussian function to the measured data and obtain the optimal values for the parameters ($A, k, \mu, \sigma$).


The uncertainty of the measured beam size, $\Delta \sigma$, is defined as the error on the standard deviation, $\sigma$, which is computed from the square root of the diagonal elements of the covariant matrix.
