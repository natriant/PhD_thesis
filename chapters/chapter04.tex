...
The SPS is equpped with rotating beam wire scanners (WS) to measure the transverse beam emittance. The SPS WS system is described in detail in Ref.~\cite{BOSSER1985475, Berrig:1972478}. The working prinicple is shown in Fig.~\ref{fig:SPS_WS_ROT}. A thin wire rapidly moves across the proton beam and a shower of secondary particles is generated. Their signal is then detected by a system of scinitllator/photomultiplier outside of the beam pipe. By measuring the wire position and the photomultiplier current at it over multiple turns the transverse beam profile is reconstructed. An example of a vertical profile is shown in Fig.~\ref{fig:WS_example_V_profile} (left).
...
In general, emittance increased is expected during each wire scan, due to Coulomb multiple scattering, which has been extensivley studied in Ref.~\cite{Roncarolo:1481835}. For the energy of 270\,GeV, at which the $\CC$ experiments were performed the expected emittance growth from the WS is expected to be small. However, a conservative number of scans were carried, $\sim$ 20 scans per bunch and per plane during $\sim$ 1 hour, in order to minimise the contribution from this effect.
...
\begin{figure}
   ...
   \normalsize{\textbf{Measuring transverse beam emittance}}\\



   
   uring the MD the evolution of the horizontal emittance
was recorded with a rotational wire-scanner (519H) while
the vertical emittance evolution was recorded with a rota

tes tets tes 

   - which one will be used in the experiment and why? (past studies showed they are not destructive)
   - How to Obtain emittance from porifile: Gaussian 4p fit
   - Dispersion for H plane. (for the 2018 was very small not taken into account)
   - In out scan
