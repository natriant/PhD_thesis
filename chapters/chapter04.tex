...
In the first configuration (see Fig.~\ref{fig:sps_cycles_cc_tests} (left)), the field of the main dipole magnets ramps up so that it accelerates the bunches from the injection energy of 26\,GeV up to 270\,GeV. The bunches remain at 270\,GeV for a few seconds before being dumped. After that the field of the SPS main dipole magnets returns to the initial value and the procedure is repeated. This configuration was used for the calibration of the $\CC$ phase offset and for the beam based measurement of the voltage (see Section ..). Additionally, it was used for the preparation of the beam for the second operational mode which is called "coast".

As discussed in the Section~\ref{subsec:cern_sps}, coast is the operational mode where the bunches circulate at the machine for long periods at constant energy (maximum 270\,GeV). During coast the magnetic field of the main dipoles ramain also constant. This is illustrated in Fig.~\ref{fig:sps_cycles_cc_tests} (right), where the magnetic filed value is illustrated for a time scale equivalent to the one of the first configuration for comparison. In reality, the dipole fields remain at constant strength for longer periods (typically 30 minutes - 1 hour).  This configuration was used for the noise-induced emittance growth measurements where the objective was to study the long term emittance evolution.

The evolution of the dipole magnetic fields during the two operational configurations that were set in the SPS up for the $\CC$ tests is illustrated in Fig.~\ref{fig:sps_cycles_cc_tests}. The corresponding beam energy is also noted on the plots.
...
The experimental studies presented in this thesis, were performed with the $\CC$s operating at a fixed frequency of 400.78\,MHz and at a target peak voltage of about 1\,MV. % this is for 270 GeV.
...
It was important to ensure that during the "coast" the beam will experience the same kick from the $\CC$ each turn. In other words the SPS main RF system operating at 200\,MHz needed to be synchronous with the $\CC$ operating at 400\,MHz. Due to the larger bandwidth of the SPS main RF system the $\CC$ was used as a master. Therefore the $\CC$ was operating at a fixed frequency, while the main accelerating cavities were adjusted to the exact half of the $\CC$ frequency so that they become synchronous with the crabbing signal. 
