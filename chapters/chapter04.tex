...
It is clear from Fig.~\ref{fig:VCC_from_HT_monitor_measurement} that the reconstructed $\CC$ voltage, $\VCC(t)$, is not centered around zero. This voltage offset seems to be a result of a calibration issue of the HT monitor as it didn't change for measurements with the same $\CC$ and beam conditions. 
...
The impact of the $\CC$ phase on the voltage experienced by the beam and on the uncertainty of its measurement was also studied experimentally. The results are summarised in Fig .. . The phase is extracted from a sinusoidal fit on the reconstructed $\CC$ voltage from the HT monitor reading after the offset is removed (Fig..). The standars procedure of least square fitting is followed.

- Also show forumla


The error bars on the voltage values (Fig a) correspond to the uncertainty of the measurement, $\Delta V_{CC}$, which is caluclated as described in Section~\ref{subsec:def_amplt_uncertainty}.

The error bars on the phase indicate the error of the fit (see Appendix~\ref{non_linear_fitting}).




Clearly the, with increasing phase ...


ePlots (maybe also with horizontal error on the measured phase?):
Amplitude vs phase
Phase offset 
...
Why do you present this study here? useful for future calibration.
For completeness? as you say in the introduction of the section?